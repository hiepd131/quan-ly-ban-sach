...
namespace QuanLyBanSach.Controllers
{
    public class NguoiDungController : Controller
    {
        ...
        public ActionResult DangKy(FormCollection collection, KhachHang kh)
        {
            ...
            {
                int demTaiKhoan = data.KhachHangs.Count(n => n.TaiKhoan == taiKhoan);
                int demEmail = data.KhachHangs.Count(n => n.Email == email);
                if (demTaiKhoan > 0)
                {
                    ViewData["LoiTaiKhoan"] = "Tên tài khoản này đã được sử dụng";
                    return View();
                }
                if (demEmail > 0)
                {
                    ViewData["LoiEmail"] = "Địa chỉ email này đã được sử dụng :";
                    return View();
                }
        ...
        {
            ViewBag.ThongBao = TempData["dangKy"] != null ? TempData["dangKy"].ToString() : "";
