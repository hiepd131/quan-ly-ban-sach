...
namespace QuanLyBanSach.Controllers
{
    public class AdminController : Controller
    {
        ...
        public ActionResult Sach(FormCollection collection, Sach sach, HttpPostedFileBase fileAnhBia, int? trang)
        {
            ...
            var anhBia = fileAnhBia != null ? fileAnhBia.FileName : "";
        ...
        public ActionResult XemThongTinKH()
        {
            var kh = Session["TaiKhoanAdmin"];

            if (kh == null)
            {
                return RedirectToAction("Index", "XemThongTinKH");
            }

            var listkh = this.data.KhachHangs.ToList();
            return View(listkh);
        }
        public ActionResult SuaKH(FormCollection collection)
        {
            var kh = Session["TaiKhoanAdmin"];

            if (kh == null)
            {
                return RedirectToAction("Index", "SuaKH");
            }
            ...
            KhachHang kh1 = this.data.KhachHangs.SingleOrDefault(n => n.MaKH == Convert.ToInt32(makh));
            if (String.IsNullOrEmpty(hoten) || String.IsNullOrEmpty(diachi) || String.IsNullOrEmpty(dienthoai)|| String.IsNullOrEmpty(matkhau)||String.IsNullOrEmpty(email))
            {
                ViewData["Loi"] = "Xin nhập đầy đủ thông tin";
            }
            else
            {
                ...
                UpdateModel(kh1);
                data.SubmitChanges();
                return RedirectToAction("XemThongTinKH", "Admin");
            }
        ...
        public ActionResult SuaKH(int id)
        {
            ...
            var kh1 = this.data.KhachHangs.SingleOrDefault(n => n.MaKH == id);
            return View(kh);
        }
        public ActionResult XemDonHang()
        {
            ...
            var listdh = this.data.DonDatHangs.ToList();
            return View(listdh);
        }
